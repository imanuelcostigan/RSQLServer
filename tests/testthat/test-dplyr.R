...
test_that("copy_to works", {
  expect_error(copy_to(ss, df, temporary = FALSE), NA)
  expect_error(copy_to(ss, df, "df1", temporary = FALSE), NA)
  expect_error(copy_to(ss, df, random_table_name(temp = TRUE)), NA)
  expect_error(copy_to(ss, df, random_table_name(temp = TRUE),
